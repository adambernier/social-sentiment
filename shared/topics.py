    ...
    def predict_batch(self, texts: list[str]) -> list[tuple[int, str]]:
        ...
        for text in texts:
            ...
            for w in words:
                ...
                if w_clean.startswith('$') or w_clean.startswith('@') or 'http' in w_clean or w_clean in ('rt', 'via'):
                    continue
            ...
            hypotheses = [self.hypothesis_template.format(label) for label in self.candidate_labels]
            ...
            logits = self.session.run(None, onnx_inputs)[0] # Shape: (len(candidate_labels), 3)
